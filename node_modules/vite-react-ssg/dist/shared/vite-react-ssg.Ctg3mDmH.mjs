...
import { join, relative, isAbsolute, parse, dirname } from 'node:path';
import fs from 'fs-extra';
import { JSDOM } from 'jsdom';
import { gray, yellow, blue, dim, cyan, red, green, reset, bold, bgLightCyan } from 'kolorist';
import PQueue from 'p-queue';
import { normalizePath, resolveConfig, createLogger, build as build$1, mergeConfig, version as version$1, send, createServer } from 'vite';
import { j as joinUrlSegments, a as withTrailingSlash, r as removeLeadingSlash, c as convertRoutesToDataRoutes, w as withLeadingSlash, s as stripBase } from './vite-react-ssg.BhGKpaqQ.mjs';
...
function collectManifestItemAssets(item, base) {
  if (!item)
    return [];
  return [
    item.file,
...
class SsrModuleTracker {
  moduleIds = new AsyncLocalStorage();
  install() {
...
function createSsrModuleTrackingPlugin(root) {
  return {
...
${transformed}`,
  ...
  async function findStaticDynamicImports(code, importer, root2) {
    ...
    await walk(ast, async (node) => {
      ...
      if (!resolved || !isTrackableSource(resolved.id, root2))
        return;
      imports.push({
        start: node.start,
        end: node.end,
        moduleId: toManifestId(resolved.id, root2)
      });
    });
    return imports;
  }
}
async function walk(node, visit) {
...
function isTrackableSource(id, root) {
  const cleanId = cleanUrl(id);
  return isSupportedSource(cleanId) && isInsideRoot(cleanId, root) && !cleanId.includes("/node_modules/");
}
function isSupportedSource(id) {
  return /\.[cm]?[jt]sx?$/.test(id);
}
function isInsideRoot(id, root) {
  return isAbsolute(id) && !relative(root, id).startsWith("..");
}
function toManifestId(id, root) {
